...
######## YesNo Dimensions ########
  ...
  measure: max_order_month {
    type: string
    sql: MAX(${created_month}) ;;
    #convert_tz: no
  }

  measure: min_order_month {
    type: string
    sql: MIN(${created_month}) ;;
    #convert_tz: no
  }

  measure: orders_by_females {
    type: count_distinct
    sql: ${order_id} ;;
  #  filters: [users.gender: "Female"]
  }

  measure: orders_by_males {
    type: count_distinct
    sql: ${order_id} ;;
   # filters: [users.gender: "Male"]
  }

  measure: percent_change_male_to_female {
    type: number
    sql: ${orders_by_males}/nullif(${orders_by_females},0) ;;
